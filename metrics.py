...
def confidence_metrics(df, correct_col, conf_col, n_bins=10):
  ...
  return roc_auc_score(df[correct_col],  df[conf_col]), brier_score_loss(df[correct_col],  df[conf_col]), ece(df[correct_col], df[conf_col], n_bins=10), smece(df[correct_col], df[conf_col])
