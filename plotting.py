...
def compare_logit_tree(logits_per_step, tokenizer, chosen_tokens, top_k=5, ax= None):
    """
    Visualize candidate token branches as a tree.
    ...
    nx.draw(G, pos, with_labels=False, node_size=2000, node_color=node_colours, arrows=True, ax = ax)
    node_labels = nx.get_node_attributes(G, "label", ax=ax)
...
def heatmap_results(results, ax = None):
    ...
    sns.heatmap(data=df_norm, annot=df_res, cmap="rocket", fmt=".4f", ax = ax)
    ax.set_title("Performance Heatmap (Normalized Colors per Column)")
