...
class LlamaHelper:
    ...
    def get_sequence_logits(self, prompt, use_beams = False, stop_at='\n', follow_up_prompt = None):
      ...
      if not self.inference['skip_intermediates']:
        ...
        full_probs = {}
      token_terminators = [self.tokenizer(term, return_tensors = "pt", add_special_tokens=False).input_ids[0].item() for term in self.inference['terminators']]
      ...
      if follow_up_prompt:
        with torch.no_grad():
            ...
            follow_up_logit_seq = token_logit_seq(torch.stack([s.detach().cpu().squeeze() for s in follow_up_scores]))

            follow_up_result = {
                "text": follow_up_text,
                "sequences": generated_tokens.detach().cpu().numpy(),
