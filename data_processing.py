...
# ==============================================================================
# 1. HELPER PARSERS & EXTRACTORS
# ==============================================================================

def parse_prompt_options(prompt_text):
    """
    Extracts choice letters and their option text from a formatted MCQ prompt.
    Returns a dict mapping letter -> choice text.
    E.g. {'A': 'a prediction', 'B': 'a hypothesis', 'C': 'an observation', ...}
    """
    if not prompt_text:
        return {}
        
    option_pattern = r'([A-E])[\)\.]\s*(.*?)(?=\n[A-E][\)\.]|\nPrint|\nQuestion|\nAnswer:|$)'
    matches = re.findall(option_pattern, str(prompt_text), re.DOTALL)
...
def detect_options_from_prompt(prompt_text):
    """
    Detects allowed option letters in the prompt (handles both 4-choice A-D and 5-choice A-E).
    """
    txt = str(prompt_text)
...
def realign_gold_answer(row):
    """
    Re-aligns raw gold answer targets to match shuffled options presented in row['prompts'].
    Fixes cases where options were permuted during prompt generation but gold key remained static.
    """
    prompt = str(row.get('prompts', ''))
    raw_ans = str(row.get('ans', '')).strip()
    source_row = row.get('source_row', {})
    
    if not isinstance(source_row, dict):
        source_row = {}

    prompt_options = parse_prompt_options(prompt)
    if not prompt_options:
        return raw_ans

    correct_text = None

    # Extract target text from source_row metadata if available
    choices = source_row.get('choices')
    if isinstance(choices, dict) and 'text' in choices and 'label' in choices:
        labels = [str(l).strip() for l in choices['label']]
        texts = choices['text']
        if raw_ans in labels:
            correct_text = str(texts[labels.index(raw_ans)]).strip()
    elif isinstance(choices, list):
        if raw_ans.isdigit():
            idx = int(raw_ans)
            if 0 <= idx < len(choices):
                correct_text = str(choices[idx]).strip()

    # Match target choice text against the actual option order in the prompt
    if correct_text:
        for letter, option_text in prompt_options.items():
            if option_text.lower() == correct_text.lower():
                return letter
...
def clean_mcq_strict(output_text, options_list=None, prompt_text=None):
    ...
    Prefers explicit answer statements, then falls back to first/last standalone option tokens.
    """
    if not output_text:
        return "none"
...
def evaluate_mcq_robust(response_text, gold_answer, prompt_text=None, options_list=None):
    """
    Evaluates MCQ output against gold target, handling leading spaces, 
    integer conversion (2 -> 'C'), and 5-option sets.
    """
    ...
    # Normalize integer target indices (e.g. '2' -> 'C')
...
def evaluate_gsm8k_robust(response_text, gold_answer):
    ...
    Handles \boxed{X}, #### X, and final numeric tokens.
    """
    if not response_text:
        return 0
...
def infer_task_family_from_row(row):
    """
    Infers task_family from prompt structures, source_row, or gold targets.
    """
    if row.get('task_family') and row.get('task_family') not in ['unknown', '']:
        return row['task_family']

    prompt = str(row.get('prompts', ''))
    ans = row.get('ans')

    if isinstance(ans, list) and len(ans) > 0 and 'assert' in str(ans[0]):
        return 'mbpp'
    if 'def ' in str(ans) or 'assert ' in str(ans):
        return 'mbpp'

    if "Solve the following grade school math problem" in prompt or "numerical answer" in prompt:
        return 'grade-school-math'

    if "[header]" in prompt or "[step]" in prompt or "[substeps]" in prompt:
        return 'hellaswag'

    if "_" in prompt and ("A)" in prompt or "A." in prompt) and not ("C)" in prompt or "C." in prompt):
        return 'winogrande'

    if "ARC-Challenge" in prompt or ("Question:" in prompt and "Choices:" in prompt):
...
def process_dataframe(df, dataset_config, metric_dict, self_conf=False, p_true=False, thinking=False):
    ...
    if thinking:
        ...
        df['thinking_text'] = [s[1] for s in splits]
        df['responses'] = [[s[0]] for s in splits]
        # Re-filter in case responses became empty post-split
        df = df[df['responses'].str.len() > 0].copy()
    else:
        df['thinking_text'] = None

    # 4. Handle Self-Confidence / P_True Metadata
    if self_conf:
    ...
    # 5. Task-Specific Evaluation Blocks
    graded_responses = []
    results = []
    results_bl = []
    results_em = []
    ...
    if dataset_config['task_type'] == 'translation':
        ...
        for out, ans, question in zip(df['responses'], df['ans'], df['prompts']):
            targets = ans
            response = [clean_mcq_strict(out[0], dataset_config.get("options", ["A", "B", "C", "D"]), prompt_text=question)]
            graded_responses.append(response)
            results.append(scorers.best_rouge_l(response, targets))
            results_em.append(scorers.best_em(response, targets))
            results_f1.append(scorers.best_f1(response, targets))
    ...
    # --------------------------------------------------------------------------
    # NEW BLOCK: 'mixed' (ROUTERBENCH-10K) MULTI-TASK EVALUATION
    # --------------------------------------------------------------------------
    elif dataset_config['task_type'] == 'mixed':
        # 1. Infer task family if not explicitly set
        df['task_family'] = df.apply(infer_task_family_from_row, axis=1)

        # 2. Re-align gold targets to prompt options if permuted
        df['ans'] = df.apply(realign_gold_answer, axis=1)

        for out, ans, question, task_fam, row_meta in zip(
            df['responses'], df['ans'], df['prompts'], df['task_family'], df.get('source_row', [{}]*len(df))
        ):
            resp_str = out[0] if isinstance(out, list) and len(out) > 0 else str(out)

            if task_fam in ['mmlu', 'arc-challenge', 'hellaswag', 'winogrande']:
                pred = clean_mcq_strict(resp_str, options_list=dataset_config.get("options"), prompt_text=question)
                response = [pred]
                score = evaluate_mcq_robust(resp_str, ans, prompt_text=question)
                ...
                response = [resp_str]
                tests = row_meta.get('test_list', ans) if isinstance(row_meta, dict) else ans
